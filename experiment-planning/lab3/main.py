...
def one_model_list(root):
    items = [
        i.Item(text="Интенсивность поступления заявок 1:", var=varList["lambda"], value=10),
...
def work_one(Event): 
    ...
    experiment.count_one(lam=lam, mu=mu, lam2=lam2)
    dfe_experiment.count_one(lam=lam, mu=mu,lam2=lam2)
...
def pfe_inputs(root):
    ...
    i_list_4.grid(column=1,  padx=10)
...
if __name__ == '__main__':
    ...
    dfe_experiment.grid(row=2, column=0, columnspan=2,   padx=10, pady=10)
